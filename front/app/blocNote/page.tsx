      ...
      <div className="w-full max-w-4xl mx-auto mt-10">
        <div className="flex items-center bg-gray-100 p-4 border-b border-gray-300">
          <button
            ...
            className={`p-2 mr-2 ${editor?.isActive('bold') ? 'bg-gray-300' : ''}`}
          >
            <i className="fas fa-bold"></i>
          </button>
          <button
            onClick={() => editor?.chain().focus().toggleItalic().run()}
            className={`p-2 mr-2 ${editor?.isActive('italic') ? 'bg-gray-300' : ''}`}
          >
            <i className="fas fa-italic"></i>
          </button>
          <button
            onClick={() => editor?.chain().focus().toggleUnderline().run()}
            className={`p-2 mr-2 ${editor?.isActive('underline') ? 'bg-gray-300' : ''}`}
          >
