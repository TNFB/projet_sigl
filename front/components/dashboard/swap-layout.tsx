...
import { createSwapy, SwapEvent as SwapyEvent, SwapEventHandler } from 'swapy'
...
export default function SwapLayout({
  ...
  const onSwap = (object: Record<string, string | null>) => {
    if (object) {
      ...
      console.error('Swap error: object is undefined')
    }
  }
...
interface CustomSwapEvent extends SwapyEvent {
  data?: {
...
export const Container = ({
  ...
  useEffect(() => {
    swapy.current?.onSwap((event: CustomSwapEvent) => {
      if (event.data && event.data.object) {
        onSwap(event.data.object)
      } else {
        console.error(
          'Swap error: event.data or event.data.object is undefined',
        )
      }
    })
  }, [onSwap])
